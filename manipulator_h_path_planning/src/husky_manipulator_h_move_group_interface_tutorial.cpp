...
int main(int argc, char **argv)
{
  ...
  visual_tools.publishText(text_pose, "Husky + Manipulator-H MoveGroupInterface tests", rvt::WHITE, rvt::XLARGE);
  ...
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //
  // The path planned under constraints only works if, for example, the
  ...
  // example the initial one), the start pose has to satisfy this orientation. So,
  ...
  ocm.header.frame_id = "base_link"; // "base_link" OR "odom", NOT SURE ABOUT THIS! WORKS BOTH WAYS!
  ...
  */
  ...
  box_pose.position.x = -0.221;
  box_pose.position.y = 0.59;
  box_pose.position.z = 0.858;
  ...
  // Read the URDF file
  std::ifstream ifss("/home/josebrito/catkin_ws/src/brito_thesis/gazebo_objects/box.urdf");
  ...
  object.request.reference_frame = "base_link";
