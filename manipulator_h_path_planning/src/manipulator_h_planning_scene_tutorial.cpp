...
// userCallback for user defined constraints
//
...
// user-defined callback that checks whether the "joint1" of
// the Robotis Manipulator-H robot is at a positive or negative angle:
...
{
  ros::init(argc, argv, "manipulator_h_planning_scene_tutorial");
  ...
  // PlanningScene.
  //
  // The :planning_scene_monitor:`PlanningSceneMonitor`  <---- IMPORTANT
  ...
  // into the collision checking function.
  //
  // Note that the result of whether the robot is in self-collision or
  // not is contained within the result. Self collision checking uses
  // an *unpadded* version of the robot, i.e. it directly uses the
  // collision meshes provided in the URDF with no extra padding added on.
  ...
  // the collision_result before making a new collision checking request.
  robot_state::RobotState& current_state = planning_scene.getCurrentStateNonConst();
  current_state.setToRandomPositions();

  // The collision_result variable has to be empty whenever we call the
  ...
  // ~~~~~~~~~~~~~~~~~~~~
  //
  // Now, we will do collision checking for the main_group of the
  // manipulator, i.e., we will check whether there are any collisions between
  // the main_group and other parts of the body of the robot.
  // We know that there won't be any but we can still ask for this specifically
  // by adding the group name "main_group" to the collision request.
  collision_request.group_name = "main_group";
  current_state.setToRandomPositions();
  collision_result.clear();
  planning_scene.checkSelfCollision(collision_request, collision_result);
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~
  //
  // First, manually set the main_group to a position where we know
  // internal (self) collisions do happen. Note that this state is now
  // actually outside the joint limits of the manipulator, which we
  // can also check for directly.
  std::vector<double> joint_values;
  const robot_model::JointModelGroup* joint_model_group = current_state.getJointModelGroup("main_group");
  current_state.copyJointGroupPositions(joint_model_group, joint_values);
  joint_values[1] = 3;  // hard-coded since we know collisions will happen here
  ...
  // have happened at a given configuration of the main group. We can ask
  ...
  // And now we again check for collisions
  collision_result.clear();
  planning_scene.checkSelfCollision(collision_request, collision_result);
  ...
  // Allow collisions for the parts of the robot that were in a collision state
  ...
  // currently empty).
  //
  // This is the set of collision checking
  ...
  //
  // We will first look at an example with a simple KinematicConstraint.
  ...
  // on the end-effector of the main group of the manipulator. Note the
  ...
  // file from the kinematic_constraints directory in moveit_core).
  ...
  desired_pose.header.frame_id = "link1";
  ...
  // There's also a direct way to do this using the KinematicConstraintSet class.
  ...
  bool state_valid = planning_scene.isStateValid(copied_state, kinematic_constraint_set, "main_group");
